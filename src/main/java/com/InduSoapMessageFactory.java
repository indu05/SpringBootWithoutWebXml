...
@Component
public class InduSoapMessageFactory  extends SaajSoapMessageFactory{
	


	private static final Log logger = LogFactory.getLog(InduSoapMessageFactory.class);
	...
	public InduSoapMessageFactory() {
	}

	/** Constructor that takes a message factory as an argument. */
	public InduSoapMessageFactory(MessageFactory messageFactory) {
				...
				for (Iterator<String> headerValues = transportInputStream.getHeaders(headerName); headerValues.hasNext();) {
					...
					while (tokenizer.hasMoreTokens()) {
						if(headerName.equalsIgnoreCase("content-type")){
							mimeHeaders.addHeader(headerName, "application/x-www-form-urlencoded");//"text/xml;charset=UTF-8");
						}else{
